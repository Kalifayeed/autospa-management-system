...
import { Clock, Tag, Scissors, Plus, Pencil } from "lucide-react";
...
export default function ServicesPage() {
  const { services, addOns, addService, updateService, addAddOn, updateAddOn } = useAppState();
  const { user } = useAuth();
  const isAdmin = user?.role === "admin";

  const categories = ["Sedan", "SUV", "Bike"];

  // Service dialog — for adding new or editing price only
  ...
  const openAddService = () => {
    setEditingSvc(null);
    setSvcName(""); setSvcCategory(categories[0]); setSvcPrice(""); setSvcDuration(""); setSvcDesc("");
    setSvcOpen(true);
  };

  const openEditPrice = (svc: Service) => {
    setEditingSvc(svc);
  ...
  const handleSaveService = async () => {
    if (editingSvc) {
      // Only update price
      await updateService(editingSvc.id, { price: Number(svcPrice) });
  ...
  const handleSaveAddon = async () => {
    if (!addonPrice) { toast.error("Price is required"); return; }
    if (editingAddon) {
      await updateAddOn(editingAddon.id, { price: Number(addonPrice) });
      toast.success("Price updated");
    } else {
      if (!addonName) { toast.error("Name is required"); return; }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm">View and manage your service catalog</p>
        </div>
        {isAdmin && (
                ...
                <div key={service.id} className="glass-card rounded-xl p-4 animate-fade-in">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      {isAdmin && (
                        <button onClick={() => openEditPrice(service)} className="h-7 w-7 rounded-md flex items-center justify-center hover:bg-secondary" title="Edit price">
      ...
      {/* Any extra categories not in the default list */}
      {[...new Set(services.map(s => s.category))].filter(c => !categories.includes(c)).map(cat => {
        const catServices = services.filter(s => s.category === cat);
        return (
          <div key={cat}>
            <h2 className="font-display font-semibold text-foreground mb-3 flex items-center gap-2">
        ...
        <div className="flex flex-wrap gap-2">
          {addOns.map(addon => (
            <div key={addon.id} className="glass-card rounded-lg px-4 py-2.5 text-sm flex items-center gap-2">
              ...
              {isAdmin && (
                <button onClick={() => openEditAddon(addon)} className="h-6 w-6 rounded flex items-center justify-center hover:bg-secondary" title="Edit price">
      ...
      <Dialog open={svcOpen} onOpenChange={setSvcOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>{editingSvc ? `Edit Price — ${editingSvc.name}` : "Add New Service"}</DialogTitle></DialogHeader>
          <div className="space-y-4 pt-2">
            {!editingSvc && (
              <>
                <div className="space-y-2"><Label>Name *</Label><Input value={svcName} onChange={e => setSvcName(e.target.value)} placeholder="e.g. Premium Wash Sedan" /></div>
            ...
            <div className={editingSvc ? "" : "grid grid-cols-2 gap-3"}>
            ...
            <Button onClick={handleSaveService} className="w-full">{editingSvc ? "Update Price" : "Add Service"}</Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={addonOpen} onOpenChange={setAddonOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>{editingAddon ? `Edit Price — ${editingAddon.name}` : "Add New Add-On"}</DialogTitle></DialogHeader>
          <div className="space-y-4 pt-2">
            {!editingAddon && (
            ...
            <Button onClick={handleSaveAddon} className="w-full">{editingAddon ? "Update Price" : "Add Add-On"}</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
